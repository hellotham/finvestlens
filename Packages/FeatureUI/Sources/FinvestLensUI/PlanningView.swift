...
/// This period against the immediately-preceding one, category by category,
/// movers called out, in plain language.
struct SpendingInsightsView: View {
    @Bindable var model: AppModel
    @State private var period: ReportPeriod = .currentMonth

    private var code: String { model.reportCurrency.mnemonic }
    private func money(_ value: Decimal) -> String { AmountFormat.string(value, code: code) }

    var body: some View {
        Group {
            if let insights = model.spendingInsights(period: period) {
                List {
                    Section {
                        ForEach(insights.summary(format: money), id: \.self) { line in
                            Label {
                                Text(line)
                            } icon: {
                    ...
                    comparisonSection("Spending by category", rows: insights.expenses,
                                      totalCurrent: insights.totalSpendingCurrent,
                                      totalPrior: insights.totalSpendingPrior)
                    comparisonSection("Income", rows: insights.income,
                                      totalCurrent: insights.totalIncomeCurrent,
                                      totalPrior: insights.totalIncomePrior)
                }
            } else {
                ContentUnavailableView("No book open", systemImage: "text.magnifyingglass")
            }
        }
        .toolbar {
            ToolbarItem {
    ...
    @ViewBuilder
    private func comparisonSection(_ title: String, rows: [CategoryComparison],
                                   totalCurrent: Decimal, totalPrior: Decimal) -> some View {
        if !rows.isEmpty {
            Section("\(title) — this period vs the one before") {
                ForEach(rows) { row in
                    HStack {
                        Text(row.name)
                        if row.isNew {
                            Text("new").scaledFont(.caption2).padding(.horizontal, 6).padding(.vertical, 1)
                                .background(.blue.opacity(0.2), in: Capsule())
                        }
                        Spacer()
                        Text(money(row.current)).monospacedDigit()
                        deltaLabel(row)
                ...
                HStack {
                    ...
                    Spacer()
                    Text(money(totalCurrent)).monospacedDigit().fontWeight(.semibold)
                    Text(money(totalCurrent - totalPrior))
                        .monospacedDigit()
                        .scaledFont(.caption)
                        .foregroundStyle(.secondary)
                        .frame(width: 110, alignment: .trailing)
    ...
    @ViewBuilder
    private func deltaLabel(_ row: CategoryComparison) -> some View {
        if row.delta == 0 {
            ...
            HStack(spacing: 2) {
                ...
                Text(row.deltaPercent.map { "\(SpendingInsights.wholePercent(abs($0)))%" }
                     ?? money(abs(row.delta)))
            }
            .scaledFont(.caption)
            .monospacedDigit()
            .foregroundStyle(up ? Color.red : .green)
        }
    }
}
