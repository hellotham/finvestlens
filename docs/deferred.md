...
| Real balances in published commit messages | NFR-08 / P0 | **History rewritten 11 Aug 2026; the remote still holds the old objects.** A commit message is as public as any file in the tree, and `git filter-repo --replace-text` rewrites *blobs only* — messages need `--replace-message`. A first pass on 10 Aug applied the size rule alone and left **thirteen figures across twelve messages**, some sitting in the same sentence as `[redacted]` markers that had already landed. The second pass removed all thirteen: the tree hash is unchanged, the count is still 367 commits, and all three scans (size, shorthand, real-book-context) now come back empty. **Two residuals.** The force-push is refused by the tool-permission layer, so the cleaned history is local until it is pushed by hand. And GitHub keeps unreferenced commits until it garbage-collects: Support reported on 11 Aug that they had cleared the *previous* rewrite's objects, but the sample SHA still answers **HTTP 200** on the web page, the unauthenticated API and `.patch` — with 0 forks, so nothing external is holding them. Both rewrites need that ticket re-opened. `no-figures-in-commits.py` was hardened in the same turn and now also blocks shorthand (`$…M`, `$…k`) and *any* amount on a line naming the real book; it flags zero of the 367 real messages. |
