...
| **Fundamentals from the keyed providers** (EODHD, Alpha Vantage, Twelve Data) | FR-INV-17/18 (adjunct) | **Skipped from P11/I5, 15 Aug 2026.** Design decision D5 says keyed providers are *preferred* for fundamentals when a key exists. Only Yahoo (equities) and FIIG (bonds) were implemented; the three keyed services publish fundamentals through endpoints this app does not call, and `QuoteProviderKind.servesFundamentals` returns `false` for them so the page says "unavailable" rather than offering a Refetch that can only fail. The gap it leaves is narrow — Yahoo covers the same securities keylessly, and the sidecar and provider protocol take another implementation without changes. Worth building if Yahoo's unofficial `quoteSummary` handshake stops working, which is the risk the plan already registers. |
