...
| iOS document flows | New/Open panels are AppKit on macOS; iOS uses fileImporter. First run in the simulator 14 Jul 2026: welcome → Open… → Files picker → book opens and renders. **New Book fixed (14 Jul 2026):** iOS books are created in the app's Documents directory (visible in Files under "On My iPhone ▸ finvestlens") with non-colliding Untitled/Untitled 2 naming — no longer the purgeable temporary directory. Still open: a move/rename flow for new books, and **GnuCash import has no iOS entry point** (the welcome link and File-menu item are macOS-only AppKit panels) — needs a fileImporter + save-location flow. |
