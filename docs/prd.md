...
| FR-NAV-02 | **The mode selector lives in the window toolbar** and is visible at all times. Not a split-view rail: a sidebar is expected to be hideable, and a mode indicator that disappears with it defeats its purpose. One segmented control, never allowed to reach the system's overflow menu; icons only at narrow widths. | Must | P12 |
