...
| FR-ACC-02 | **Placeholder and hidden cascade down a subtree.** Setting either on a parent offers to apply it to every account beneath it, in one undoable action — a chart of accounts is reorganised by branch, and doing it row by row is where a half-hidden subtree comes from. | Should | P2 |
| FR-ACC-03 | **Open Subaccounts**: a parent's register may show the postings of its whole subtree in one register, not only its own. GnuCash's *Open Subaccounts*, and the reason the register's summary and entry bar have to know which it is showing. | Should | P2 |
| FR-ACC-04 | **Deleting an account says where its contents go.** An account with postings, subaccounts, or both cannot simply vanish: the delete asks which account inherits the transactions and which the children, and refuses where no valid destination exists. | Must | P2 |
...
| FR-SEC-01 | **Secrets live in the system Keychain**, never in the document, never in `UserDefaults`, and never in a log or an error message. Quote API keys are the only secrets the app holds; the store is protocol-backed so tests can substitute an in-memory one without a Keychain entitlement. | Must | P5 |
...
| FR-INV-42 | Price **unlisted managed funds and trusts** whose unit prices are published only on the manager's own website, keyed by the fund's page slug rather than a ticker, and read the fund's profile — inception, asset class, benchmark, timeframe, APIR, ARSN, fees — from the same request. Such a page is written for people, not machines: its published dates may be inconsistent within one table (day-first above the 12th, month-first below), so the parser resolves the whole column to a monotonic series rather than trusting any single row. Implemented for Wilson Asset Management. | Should | P13 |
...
### 5.15a Navigation: modes, sidebar, tabs (P12)

The Jul 2026 redesign moved feature areas out of modal launchers and into the
sidebar, which was right and is not reversed here. What it left behind is a
sidebar that is **areas and collections at once** — thirteen destinations above
a 565-account tree — so the account list is present in Reports, absent the
moment you type in the filter, and pinned to the window's bottom edge. Design,
alternatives and the guidance behind each decision: [navigation-design.md](navigation-design.md).

| ID | Requirement | Pri | Phase |
|---|---|---|---|
| FR-NAV-01 | **Modes.** The app is organised into modes — Overview, Accounts, Investments, Reports, Business, Planning, Records — each with its own sidebar collection, its own open tabs, and its own selection, all preserved while the user is elsewhere. | Must | P12 |
| FR-NAV-02 | **The mode selector lives in the window toolbar** and is visible at all times. Not a split-view rail: a sidebar is expected to be hideable, and a mode indicator that disappears with it defeats its purpose. **One item per mode**, never allowed to reach the system's overflow menu; the name under the symbol, and the symbol alone at widths too narrow to hold the words (`ModeLabelFit`). *(This asked for "one segmented control" until Aug 2026. The build is deliberately not one: the system's own toolbar customisation — required by `FR-NAV-03`, so a mode can be added or removed — works on toolbar **items**, and a single segmented control is one item that cannot be split. The requirement was describing an implementation, and the implementation it described could not satisfy the requirement beside it.)* | Must | P12 |
| FR-NAV-03 | **Five modes on the toolbar by default** (Overview · Accounts · Investments · Reports · Business); the set is **customisable** through the system's own toolbar customisation. **Every** mode is always in the View menu with ⌘1…⌘n, so a mode absent from the toolbar is never unreachable. Modes are never hidden or disabled because their content is empty. | Must | P12 |
| FR-NAV-04 | **One sidebar per mode, showing one kind of thing**, two levels deep: a section heading over its instances. Accounts shows favourites and the account tree; Investments securities and portfolios; Reports standard/custom/favourites; Planning budgets, goals and scheduled; Business its customers, vendors and invoices; Records its rules, emergency records and audit log. No mode's sidebar is a list of commands. | Must | P12 |
| FR-NAV-05 | **A tabbed interface within the detail pane** — several open items sharing one mode sidebar. Not macOS window tabs, which carry a whole window each and would duplicate the sidebar per tab. | Must | P12 |
| FR-NAV-06 | **Opening a tab is deliberate.** A single click on a sidebar row *replaces* the current tab's content; a new tab requires double-click, ⌘-click, a context-menu "Open in New Tab", or the new-tab button. Selecting something already open **focuses its existing tab** rather than duplicating it. A placeholder account opens nothing — it expands, as it has no register. Each mode's home tab is first and not closeable. The open set is desk state, restored on open, never in the document. | Must | P12 |
| FR-NAV-07 | **Overview is the launch page** and reports across every mode. Every mode must be able to contribute at least one card, and the default board carries one from each — which requires a Business card (receivables/overdue), the one mode that contributes none today. | Must | P12 |
| FR-NAV-08 | **Overview is a board of views**, its sidebar listing views with **their cards nested underneath**: choosing a view shows that board, choosing a card shows that card full-window. Standard views plus the user's own; **a favourite is a saved custom view**, not a separate concept. | Must | P12 |
| FR-NAV-09 | **Selecting changes what you see; drilling in changes where you are.** Choosing a view or a card **never** switches mode. Clicking *through* a card to its underlying data does, as does an explicit "Open <mode>" button on the board. The mode selector must never change except by a gesture that asked to move. | Must | P12 |
| FR-NAV-10 | **Every card opens full-window** with a close button returning to the board, and a card that is not on the current view is reachable from a toolbar list that opens it full-window until closed. Nothing is unreachable for having not fitted. | Must | P12 |
| FR-NAV-11 | **One period selector, everywhere.** A single control governs the Overview board and every mode, and supplies the default period a report opens with — replacing the dashboard's private period and the reports' own. | Should | P12 |
| FR-NAV-12 | **Sidebar manners, common to every mode.** Sections sort by criterion (name, code, balance, type, and for accounts a *derived* earliest-transaction date, labelled as derived) or by hand, the manual order stored in the account's own kvp so GnuCash ignores it harmlessly. Dragging between siblings reorders; dragging **onto** a row re-parents — a book edit, undoable, and visually distinct from a reorder while dragging. Dragging is disabled while a criterion sort is active. One context menu serves selected and unselected rows alike. | Should | P12 |
...
| FR-FIND-02 | **Find Account (⌘I)**: type a few letters and land on the account, however deep it is buried — GnuCash's Find Account, for the keyboard. The sidebar filter covers browsing; this exists because browsing a 565-account tree with the mouse is not a way to reach one you can name. | Should | P2 |
| FR-FIND-03 | **Bulk operations on search results**: the rows a search returns are a selection, and the same transaction operations apply to all of them at once — categorise, reconcile, tag, edit in bulk. A result list you cannot act on is a report, not a search. | Should | P4 |
