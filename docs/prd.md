...
| FR-REG-03 | **One register, three disclosure styles** *(Aug 2026: the Jul 2026 "one register, no styles" simplification was reversed — see [register-ux-research.md](register-ux-research.md))*: a single table whose rows disclose their full detail — notes, tags and every leg. **Basic Ledger** discloses only the row you open by its triangle, **Auto Details** discloses the selected row, **Transaction Journal** discloses them all; these are GnuCash's `SplitRegisterStyle` values. Columns sort by heading click and resize by dragging the header divider. The whole-book journal is the **All Transactions** sidebar destination (FR-REG-09). | Should | P2 |
